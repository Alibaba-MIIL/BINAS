...
def flatten_expected_accuracy(list_alphas):
    '''
    Flatten all the layers expected accuracy as corresponding column stack vectors
    :param list_alphas: A list of dictionaries mapping all the modules to their corresponding attentions and latencies
    :return: alpha_expected_accuracy: A column vector of all the expected accuracy attributed to all the layers
             beta_expected_accuracy: A column vector of all the expected accuracy attributed to all the blocks
    '''
    alpha_expected_accuracy_vec = np.empty(0)
    beta_expected_accuracy_vec = np.empty(0)

    for entry in list_alphas:
        expected_accuracy = entry['expected_accuracy'].squeeze().cpu().numpy()
        if 'alpha' in entry:
            alpha_expected_accuracy_vec = np.concatenate((alpha_expected_accuracy_vec, expected_accuracy))

        if 'beta' in entry:
            beta_expected_accuracy_vec = np.concatenate((beta_expected_accuracy_vec, expected_accuracy))

    return alpha_expected_accuracy_vec, beta_expected_accuracy_vec
...
def flatten_alpha(entry, alpha_attention_vec, latency_vec, alpha_grad_vec, alpha_blocks):
    ...
    latency = entry['latency'].detach() if entry['latency'] is not None else None
    ...
    latency_vec = np.concatenate((latency_vec, latency.squeeze().cpu().numpy())) if latency is not None else None
    if alpha_grad is not None:
