...
from nas.src.optim.differential_evolution import DiffEvo
from nas.src.optim.evo import Evo
from nas.src.optim.iqcp import IQCP
...
def create_optimizer_alpha(args, list_alphas, lr):
    ...
    for entry in list_alphas:
        ...
        if 'latency' in entry:
            entry['latency'] = entry['latency'].cuda().half() if entry['latency'] is not None else None
        if 'beta' in entry:
            entry['beta'] = entry['beta'].cuda()
            entry['beta'].requires_grad = True
            parameters.append(entry['beta'])

    if args.nas_optimizer in ['block_frank_wolfe', 'bcfw']:
        optimizer = BlockFrankWolfe(parameters, list_alphas,
                                    args.inference_time_limit,
    ...
    elif args.nas_optimizer == 'iqcp':
        optimizer = IQCP(parameters, list_alphas, args.inference_time_limit, args.max_gamma_step)

    elif args.nas_optimizer == 'diff_evo':
        optimizer = DiffEvo(parameters, list_alphas, args.inference_time_limit, args.max_gamma_step)

    elif args.nas_optimizer == 'evo':
        optimizer = Evo(parameters, list_alphas, args.inference_time_limit, args.max_gamma_step)
